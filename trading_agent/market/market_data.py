...
# Cache TTLs (seconds) — seeded from trading_rules.yaml at import time.
# Importable by name for backward compatibility with tests.
from trading_agent.config.loader import CacheRules as _CacheRules
_cache_rules = _CacheRules()
PRICE_HISTORY_TTL = _cache_rules.price_history_ttl     # 4 hours
SNAPSHOT_TTL = _cache_rules.snapshot_ttl               # 1 minute
OPTION_CHAIN_TTL = _cache_rules.option_chain_ttl       # 3 minutes
INTRADAY_RETURN_TTL = _cache_rules.intraday_return_ttl # 1 minute
_MAX_PREFETCH_WORKERS = _cache_rules.max_prefetch_workers
